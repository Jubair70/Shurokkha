...
function reDraw(context, data, canEdit)
{
    ...
    if(data)
    {
        ...
        {   
            ...
            if (isArray(value))
            {
                for (var idx in value)
                {
                    ...
                    for(var sub_key in val_array)
                    {
                        ...
                        if( questions.hasOwnProperty(cleanKey)  && (questions[cleanKey].type ==='select one' || questions[cleanKey].type ==='select all that apply' ))
                        {
                            if(questions[cleanKey].type ==='select all that apply'){
                                var subArray = val_array[sub_key].split(" ");
                                var value_label_temp = '';
                                for(var i = 0; i < subArray.length; i++){
                                    for(child in questions[cleanKey].children)
                                    {
                                        if(subArray[i] == questions[cleanKey].children[child].name)
                                        {
                                            value_label_temp = value_label_temp + questions[cleanKey].children[child].label;
                                    ...
                                    value_label = value_label_temp
                            }
                            } else {
                            for(child in questions[cleanKey].children)
                            {
                                
                                    if(val_array[sub_key]==questions[cleanKey].children[child].name)
                                    {
                                        value_label = questions[cleanKey].children[child].label;   
                           }
                       }
                       }
                   }
                        ...
                        if (cleanData.hasOwnProperty(cleanKey))
                            cleanData[cleanKey] = cleanData[cleanKey]+'<br>'+value_label;
                        else
                            cleanData[cleanKey] = value_label;
                        //break;
                    }
                }
            } 
            ...
            var cleanKey = key.replace(cleanRe, '_');
            ...
            if(questions.hasOwnProperty(cleanKey))
            {
                    ...
                    else if ( questions[cleanKey].type === 'select one' || questions[cleanKey].type === 'select all that apply')
                    {
                        
                       
                            for(child in questions[cleanKey].children)
                            {
                                if(value == questions[cleanKey].children[child].name)
                                {
                                    value = questions[cleanKey].children[child].label;
                                    //console.log(questions[cleanKey].children[child].label);
                                    //break;
                                }
                            }
            ...
            if(cleanKey == '_submitted_by')
            {
                ...
                //console.log('_submitted_by: '+ data['_submitted_by']);
            } 
