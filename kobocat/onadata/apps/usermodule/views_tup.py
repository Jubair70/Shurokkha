...
from onadata.apps.tupmodule.models import Cohort, Cohortform, CohortBranch, MobileModule, MobileModuleAccess
from django.contrib.auth.models import User
...
def index(request):
    ...
    #     users = user
    #     admin = False
    #mobile_access(request, "raygonj_po1")
...
def organization_index(request):
    ...
    all_organizations = []
    if request.user.is_superuser:
        query = 'select id, organization, district, upazila, branch_code from vwgeoorganization'
        all_organizations = __db_fetch_values(query)
    else:
...
def get_hh_list(request,username):
    ...
    sql = 'with t as(SELECT hh_status_id,  hh_serial, hh_id, village_id, spot_id, hh_name, nid, hh_husband_father_name,wealth_rank,(select cohort_name from cohort where id =household.cohort_id limit 1 ) as cohort ,gps,(SELECT status FROM public.approval_instanceapproval where instance_id=household.last_instance_id limit 1) status, hh_name FROM public.household where branch_id in(%s)) select t.*, dist_code, district, up_code, upazila , union_code, union_name, village_code, village,spot_code,spot from t,vwspot where t.spot_id=vwspot.spot_id'%(org_string)
    print sql
    ...
    if len(household_list) > 0:
        for household in household_list:
            ...
            json_data_response['cohort_name'] = household[9]
            json_data_response['status'] = 'new'
	    district = household[13]
            district_label = household[14]
            upazila = district+household[15]
            upazila_label = household[16]
            union = upazila+household[17]
            union_label = household[18]
            village = union+household[19]
            village_label = household[20]
	    json_data_response['pra_id'] = village+household[21]
            json_data_response['pra_name'] = household[22]
	    json_data_response['village_name'] = village_label
            ...
            #last visit date
            visit_plan_query = "with t as(select id,json_array_elements((json->>'add_vo')::json) json from vwlogger_instance where xform_id = 482), t1 as(select id,json->>'add_vo/vo_id' vo_id, json_array_elements((json->>'add_vo/add_hh')::json) json from t) select id,vo_id,json->>'add_vo/add_hh/date' plan_date, json->>'add_vo/add_hh/hh_id' hh_id from t1 where json->>'add_vo/add_hh/hh_id' = '%s'" % (
            household[2])
            print visit_plan_query
            visit_plan = __db_fetch_values(visit_plan_query)
            print visit_plan
            json_data_response['visit_plan_date'] = ''
            if len(visit_plan)>0:
                json_data_response['visit_plan_date'] = visit_plan[0][2]
            json_data_response['form_id'] = 'hh_registration'
            
            json_data_response = household_data(household[2], json_data_response)
            print json_data_response
...
def get_hh_list(request, username):
    ...
    #     spot_string = "'%'"
    # sql = 'SELECT hh_status_id,  hh_serial, hh_id, village_id, spot_id, hh_name, nid, hh_husband_father_name,wealth_rank,mobile,gps FROM public.household where village_id in (%s)'%(spot_string)
    sql = 'with t as(SELECT hh_status_id,  hh_serial, hh_id, village_id, spot_id, hh_name, vo_id, hh_husband_father_name,wealth_rank,(select cohort_name from cohort where id =household.cohort_id limit 1 ) as cohort ,gps,(SELECT status FROM public.approval_instanceapproval where instance_id=household.last_instance_id limit 1) status, household.last_instance_id FROM public.household where branch_id in(%s)) select t.*, dist_code, district, up_code, upazila , union_code, union_name, village_code, village,spot_code,spot from t,vwbranchspotcoverage v where t.spot_id=v.spot_id' % (
    org_string)
    print sql
    household_list = __db_fetch_values(sql)
    json_data_response_all = []
    if len(household_list) > 0:
        for household in household_list:
            ...
            json_data_response['husband_father_name'] = household[7]
            json_data_response['wealth_rank'] = household[8]
            wealth_rank_query = "select value_label::json->>'Bangla' from xform_extracted where xform_id = 459 and field_name like 'wealth_rank' and value_text = '%s'"%(household[8])
	    json_data_response['wealth_rank'] = row_query(wealth_rank_query)
            json_data_response['cohort_name'] = household[9]
            json_data_response['status'] = 'new'
            json_data_response['instance_id'] = household[12]
            json_data_response['location'] = household[10]
            district = household[13]
            district_label = household[14]
            upazila = district + household[15]
            upazila_label = household[16]
            ...
            json_data_response['form_id'] = 'hh_registration'
            '''last visit date'''
            visit_plan_query = "select json->>'date' from vwlogger_instance where xform_id = 484 and json->>'hh_id'='%s' order by json->>'_submission_time' desc" % (household[2])
            print visit_plan_query
            visit_plan = __db_fetch_values(visit_plan_query)
            print visit_plan
            json_data_response['visit_plan_date'] = ''
            if len(visit_plan) > 0:
                json_data_response['visit_plan_date'] = visit_plan[0][0]
            '''vo_id'''
            # vo_query = " %s"%(household[2])
            json_data_response['vo_id'] =household[6]
            # print 'row query'
            # print row_query(vo_query)
            # json_data_response['vo_id'] = row_query(vo_query)
            '''home_visit'''
            json_data_response['last_visit_date'] = ''

            last_visit_query = "select  json->>'visit_date' from vwlogger_instance where xform_id = 439 and json->>'hh_id'='%s' order by json->>'visit_date' asc "%(household[2])
            last_visit_date = __db_fetch_values(last_visit_query)
            home_visit_count = len(last_visit_date)
            json_data_response['visit_count'] = home_visit_count
            if home_visit_count>0:
                json_data_response['last_visit_date'] = last_visit_date[home_visit_count-1][0]
            json_data_response = household_data(household[2], json_data_response)
            json_data_response = input_asset_data(household[2], json_data_response)
            # json_data_response
            json_data_response_all.append(json_data_response)
    # print json.dumps(json_data_response)
    return HttpResponse(json.dumps(json_data_response_all))
...
def get_village_list(request,username):
    ...
    if len(village_list) > 0:
        for village in village_list:
            json_data_response = {}
            json_data_response['village_code'] = village[0]
	    ...
	    json_data_response['status'] = 'new'
            json_data_response['form_id'] = 'add_village'
            json_data_response_all.append(json_data_response)
...
def get_pra_list(request,username):
    ...
    if len(pra_list) > 0:
        for village in pra_list:
            ...
            json_data_response['union_name'] = village[7]
            json_data_response['status'] = 'new'
            json_data_response['form_id'] = 'add_pra'
            json_data_response_all.append(json_data_response)
...
def get_input_list(request,username):
    ...
    if len(input_list) > 0:
		...
		json_data_response['input_name'] = input_item[5]
                ...
                input_type = input_item[2].split(" ")
                name = []
                for i in input_type:
		    input_name_query = "select value_label from xform_extracted where xform_id = 444 and field_name = 'input_type' and value_text = '%s'" % (i)
                    print input_name_query
                    input_name_value = json.loads(row_query(input_name_query))
                    name.append(input_name_value['Bangla'])
                json_data_response['input_name'] = ' '.join(str(x.encode('utf-8')) for x in name)
...
@csrf_exempt
def get_asset_list(request,username):
    
    #username = request.GET.get('username', '')

    sql = "select json->>'info/hh_id', json->>'_xform_id_string', json->>'asset_type', json->>'asset_value', json->>'date_asset_distribution', json->>'asset_name', json->>'asset_quantity', (SELECT status FROM public.approval_instanceapproval where instance_id=logger_instance.id limit 1) status from logger_instance where xform_id=471 and user_id=(select id from auth_user where username='" + username + "') and deleted_at is null order by id"
    ...
    if len(input_list) > 0:
        for input_item in input_list:
            ...
            json_data_response['asset_type'] = input_item[2]
            asset_type_query = "select value_label::json->>'Bangla' from xform_extracted where xform_id=471 and field_name = 'asset_type' and value_text = '%s'"%(input_item[2])
            json_data_response['asset_type_name']=row_query(asset_type_query)
            json_data_response['asset_value'] = input_item[3]
            json_data_response['asset_distribution_date'] = input_item[4]
            json_data_response['asset_name'] = input_item[5]
            asset_name_query = "select value_label::json->>'Bangla' from xform_extracted where xform_id=471 and field_name = 'asset_name' and value_text = '%s'"%(input_item[5])
            json_data_response['asset_name_name'] = row_query(asset_name_query)
	    json_data_response['asset_quantity'] = input_item[6]

            json_data_response['status'] = input_item[7]
            json_data_response_all.append(json_data_response)
...
def get_training_list(request, username):
    substring = ''
    # username = request.GET.get('username', '')
    if get_user_role(username) == 'PO':
        substring = " and json->>'assign_po'='" + username + "'"
    if get_user_role(username) == 'BM':
        substring = " and user_id=(select id from auth_user where username='" + username + "')"

    sql = "select id, json->>'_xform_id_string', json->>'enterprise_name', json->>'enterprise_option', json->>'training_start_date', json->>'training_end_date', json->>'assign_po', json->>'group', (SELECT member_list FROM public.training_member where training_id=logger_instance.id::text limit 1) member_list, (SELECT status FROM public.approval_instanceapproval where instance_id=logger_instance.id limit 1) status from logger_instance where xform_id=465 " + substring + " and deleted_at is null order by id"
    
    print sql
    training_list = __db_fetch_values(sql)
    json_data_response_all = []
    if len(training_list) > 0:
        for training in training_list:
            json_data_response = {}
            json_data_response['training_id'] = training[0]
            json_data_response['form_id'] = training[1]
            json_data_response['training_start_date'] = training[4]
            json_data_response['training_end_date'] = training[5]
            json_data_response['assign_po'] = training[6]
            query_po = "select first_name, last_name from auth_user where username='" + training[6] + "'"
            name = __db_fetch_values(query_po)
            json_data_response['assign_po_name'] = name[0][0] + ' ' + name[0][1]
            json_data_response['group_id'] = int(training[7])
            if training[8] is not None:
                json_data_response['member_list'] = json.loads(training[8])
            else:
                json_data_response['member_list'] = []

            json_data_response['status'] = training[9]

            enterprise_name_query = "select name from  enterprise where code = %s" % (int(training[2]))
            enterprise_name = row_query(enterprise_name_query)
            json_data_response['enterprise_name_id'] = int(training[2])
            json_data_response['enterprise_name_name'] = enterprise_name

            if training[3] is not None:
                enterprise_option_query = "select option_name from  enterprise_option where code = %s" % (
                int(training[3]))
                enterprise_option = row_query(enterprise_option_query)
                json_data_response['enterprise_option_id'] = int(training[3])
            ...
            json_data_response['group_name'] = group_value['Bangla']

            '''visit '''
            json_data_response['last_visit_date'] = ''
            last_visit_query = "select  json->>'date', json->>'hh_id' from vwlogger_instance where xform_id = 469 and json->>'training_id' = '%s' order by json->>'date' asc" % (training[0])

            print last_visit_query
            last_visits = __db_fetch_values(last_visit_query)
            print last_visits
            visit_count = len(last_visits)
            json_data_response['visit_count'] = visit_count
            if visit_count > 0:
                json_data_response['last_visit_date'] = last_visits[visit_count - 1][0]
            attendance = []
            for visit in last_visits:
                dict = {}

                dict['date'] = visit[0]
                dict['participants'] = visit[1].split(' ')
                attendance.append(dict)
            json_data_response['attendance_list'] = attendance
...
@csrf_exempt
def get_vo_list(request,username):
    ...
    sql = "select id, json->>'_xform_id_string', json->>'vo_id', json->>'vo_name', json->>'assign_po', (SELECT member_list FROM public.group_visit_member where group_visit_id=logger_instance.id::text limit 1) member_list, (SELECT status FROM public.approval_instanceapproval where instance_id=logger_instance.id limit 1) status from logger_instance where xform_id=468 " +substring + " and deleted_at is null order by id"
    
    group_visit_list = __db_fetch_values(sql)
    json_data_response_all = []
    if len(group_visit_list) > 0:
        for group_visit in group_visit_list:
            json_data_response = {}
            json_data_response['group_visit_id'] = group_visit[0]
            json_data_response['form_id'] = group_visit[1]
            json_data_response['vo_id'] = group_visit[2]
            json_data_response['vo_name'] = group_visit[3]
            json_data_response['assign_po'] = group_visit[4]
            query_po = "select first_name, last_name from auth_user where username='"+group_visit[4]+"'"
            name = __db_fetch_values(query_po)
            json_data_response['assign_po_name'] = name[0][0]+' '+name[0][1]
            print type(group_visit[5])
            if group_visit[5] is not None:
                json_data_response['member_list'] = json.loads(group_visit[5])
            else:
                json_data_response['member_list'] = []

            '''visit '''
            json_data_response['last_visit_date'] = ''
            last_visit_query = "select  json->>'date', json->>'beneficiary_name' from vwlogger_instance where xform_id = 474 and json->>'vo_id' = '%s' order by json->>'date' asc"%(group_visit[2])
            ...
            '''visit plan date'''
            json_data_response['visit_plan_date'] = ''
            query =" with t as(select id,json_array_elements((json->>'visit')::json) json from vwlogger_instance where xform_id = 476) select json->>'visit/add_vo/date' from t where  json->>'visit/add_vo/vo_id' = '%s'"%(group_visit[2])
            print query
            visit_plan = __db_fetch_values(query)
            print visit_plan
            if len(visit_plan)>0:
                json_data_response['visit_plan_date'] = visit_plan[0][0]
            json_data_response['status'] = group_visit[6]
            json_data_response_all.append(json_data_response)
...
def get_hh_datalist(request,username,hhid):
    ...
    sql = "select xml xml_content,json->>'_xform_id_string' form_id, json->>'meta/instanceID' instance_id, to_char(date_created,'DD-Mon-YYYY HH:mm') created_date,(select title from public.logger_xform where id = logger_instance.xform_id limit 1) form_name, (select concat(first_name, ' ', last_name) from auth_user where username=json->>'_submitted_by') user_name from logger_instance where (json->>'hh_id'='" + hhid + "' or json->>'info/hh_id'='" + hhid + "') and deleted_at is null order by id"
    ...
    if tmp_db_value is not None:
       ...
       for every in tmp_db_value: 
           print every[0]
           instance_data_json = {}
           instance_data_json['xml_content'] = str(every[0])
           instance_data_json['form_id'] = str(every[1].encode('utf-8'))
           instance_data_json['instance_id'] = str(every[2])
           instance_data_json['created_date'] = str(every[3])
           instance_data_json['form_name'] = str(every[4])
           instance_data_json['user_name'] = str(every[5])
...
def get_hh_profile(request,username,hhid):
    ...
    #select id,json->>'_xform_id_string' id_string,date_created from logger_instance
    '''
    ben_query = "select json->>'husband_father_name' , json->>'beneficiary_name', json->>'member_info', json->>'mobile', json->>'group_selected_assessment', json->>'picture', (select username from auth_user where id = user_id), json->>'gps' from vwlogger_instance where xform_id = 457 and json->>'info/hh_id' = '%s'" % (
        hhid)
    print ben_query
    enterprise_query = "select json->>'enterprise_name',json->>'enterprise_option',json->>'enterprise_type' from vwlogger_instance where xform_id = 464 and json->>'info/hh_id' = '%s'" % (
        hhid)
    group_query = "select json->>'group'::text , (select username from auth_user where id = user_id) from vwlogger_instance where xform_id = 458 and json->>'info/hh_id' = '%s'" % (
        hhid)
	...
	json_data_response['location'] = bene_list[0][7]
        ben_username=bene_list[0][6]
    # group selection
    group_list = __db_fetch_values(group_query)
    if 'group_selected_assessment' in json_data_response:
	...
	i = 1
        for g in group_list:
            ...
            group_json = row_query(group_value_query)
            print group_json
	    if group_json is not None:
		group_value = json.loads(group_json)
		if i == length:
    ...
    if len(enterprise) > 0:
        ...
        if enterprise[0][0] is not None:
            ...
            json_data_response['enterprise_option_name'] = enterprise_option
    '''
    json_data_response=household_data(hhid, json_data_response)
    return HttpResponse(json.dumps(json_data_response))



def household_data(hhid, json_data_response):
    print 'inside household data'
    ben_query = "select json->>'husband_father_name' , json->>'beneficiary_name', json->>'member_info', json->>'mobile', json->>'group_selected_assessment', json->>'picture', (select username from auth_user where id = user_id), json->>'gps',json->>'type_guardian'  from vwlogger_instance where xform_id = 483 and json->>'info/hh_id' = '%s'" % (
        hhid)
    print ben_query
    enterprise_query = "select enterprise_name,enterprise_option,enterprise_type, username from household_enterprise where hh_id = '%s'" % (hhid)
    group_query = "select json->>'group'::text , (select username from auth_user where id = user_id) from vwlogger_instance where xform_id = 458 and json->>'info/hh_id' = '%s'" % (
        hhid)
    bene_list = __db_fetch_values(ben_query)
    #json_data_response = {}
    if len(bene_list) > 0:
        ...
        member_info = json.loads(bene_list[0][2])
        #json_data_response['member'] = json.loads(bene_list[0][2])
        json_data_response['member'] = member_info_data(json.loads(bene_list[0][2]))
        json_data_response['mobile'] = bene_list[0][3]
        json_data_response['group_selected_assessment'] = bene_list[0][4]
        json_data_response['picture'] = bene_list[0][5]
	json_data_response['location'] = bene_list[0][7]
        json_data_response['type_guardian'] = bene_list[0][8]
        ben_username=bene_list[0][6]
    # group selection
    group_list = __db_fetch_values(group_query)
    if 'group_selected_assessment' in json_data_response:
    ...
    if len(enterprise) > 0:
        for enter in enterprise:
            json_data = {}
            role = str(get_user_role(enter[3]).encode('utf-8'))
            if enter[2] is not None:
                enterprise_type_query = "select label from  enterprise_type where id = %s" % (int(enter[2]))
                enterprise_type = row_query(enterprise_type_query)
                json_data['enterprise_type_id'] = int(enter[2])
                json_data['enterprise_type_name'] = enterprise_type
            if enter[1] is not None:
                enterprise_name_query = "select name from  enterprise where code = %s" % (int(enter[0]))
                enterprise_name = row_query(enterprise_name_query)
                json_data['enterprise_name_id'] = int(enter[0])
                json_data['enterprise_name_name'] = enterprise_name
            if enter[0] is not None:
                enterprise_option_query = "select option_name from  enterprise_option where code = %s" % (int(enter[1]))
                enterprise_option = row_query(enterprise_option_query)
                json_data['enterprise_option_id'] = int(enter[1])
                json_data['enterprise_option_name'] = enterprise_option
            role_enterprise[role] = json_data
        
        if 'RM' in role_enterprise:
            json_data_response = dict(json_data_response.items()+role_enterprise['RM'].items())
            print "RM selected"
        elif 'BM' in role_enterprise:
            json_data_response = dict(json_data_response.items() + role_enterprise['BM'].items())
            print "BM selected"
        else:
            json_data_response = dict(json_data_response.items() + role_enterprise['PO'].items())
            print "PO selected"

    return json_data_response


def input_asset_data(hhid, json_data_response):

    #household_input
    # input_query = "select  json->>'input_type', json->>'value_tk', json->>'date_input_distribution', json->>'input_type_other' from vwlogger_instance where xform_id=444 and json->>'info/hh_id' = '%s'"%(hhid)
    # input_data = __db_fetch_values(ben_query)
    # json_data_response_all = []
    # if len(input_list) > 0:
    #     for input_item in input_list:
    #         json_data_response = {}
    #         json_data_response['input_type'] = input_item[0]
    #         json_data_response['input_value'] = input_item[1]
    #         json_data_response['input_distribution_date'] = input_item[2]
    #         if input_item[3] is not None:
    #             json_data_response['input_name'] = input_item[3]
    #         else:
    #             input_name_query = "select value_label from xform_extracted where xform_id = 444 and field_name = 'input_type' and value_text = '%s'" % (
    #             input_item[0])
    #             input_name_value = json.loads(row_query(input_name_query))
    #             json_data_response['input_name'] = input_name_value['Bangla']
    #     json_data_response_all.append(json_data_response)

    sql = "select json->>'asset_type', json->>'value_tk', json->>'date_asset_distribution', json->>'asset_name', json->>'asset_quantity' from vwlogger_instance where xform_id=471 and json->>'info/hh_id' = '%s' order by json->>'date_asset_distribution' desc limit 1"%(hhid)

    asset_list = __db_fetch_values(sql)
    # json_data_response_all = []
    if len(asset_list) > 0:
        for input_item in asset_list:
            json_data_response['asset_type'] = input_item[0]
            asset_type_query = "select value_label::json->>'Bangla' from xform_extracted where xform_id=471 and field_name = 'asset_type' and value_text = '%s'"%(input_item[0])
            json_data_response['asset_type_name']=row_query(asset_type_query)
            json_data_response['asset_value'] = input_item[1]
            json_data_response['asset_distribution_date'] = input_item[2]
            json_data_response['asset_name'] = input_item[3]
            asset_name_query = "select value_label::json->>'Bangla' from xform_extracted where xform_id=471 and field_name = 'asset_name' and value_text = '%s'"%(input_item[3])
            json_data_response['asset_name_name'] = row_query(asset_name_query)
            #json_data_response['asset_name'] = input_item[]
            json_data_response['asset_quantity'] = input_item[4]


    return json_data_response
'''
def member_info_data(member_list):

    renew_data = []
    for d in member_list:
        dict = d
        print d
        if d['member_info/gender']=='1':
            d['member_info/gender'] = 'Female'
        if d['member_info/gender']=='2':
            d['member_info/gender'] = 'Male'
        if 'member_info/maritial_status' in d and d['member_info/maritial_status']=='1':
            d['member_info/maritial_status'] = 'Married'
        if 'member_info/maritial_status' in d and d['member_info/maritial_status']=='2':
            d['member_info/maritial_status'] = 'Unmarried'
        relation_query = "select value_label from xform_extracted where field_name='member_info/relation' and value_text= '%s' and xform_id = 483"%(d['member_info/relation'])
        relation = eval(row_query(relation_query))
        d['member_info/relation'] = str(relation['Bangla'].encode('utf-8'))
        #occupation 
        #occupation = d['member_info/involement_iga'].split(' ')
        involement_iga_query = "select value_label from xform_extracted where field_name='member_info/involement_iga' and value_text= '%s' and xform_id = 483"%(d['member_info/involement_iga'])
        #involement_iga = eval(row_query(involement_iga_query))
        #d['member_info/involement_iga'] = str(involement_iga['Bangla'].encode('utf-8'))
        print "#######################3"
        print d
        renew_data.append(d)
    return renew_data
...
def member_info_data(member_list):
    ...
    for d in member_list:
        ...
        # if 'member_info/maritial_status' in d and d['member_info/maritial_status']=='2':
        #     d['member_info/maritial_status'] = 'Unmarried'
        if 'member_info/maritial_status' in d:
            single_query = "select field_detail->>'label' from xform_extracted where field_name='member_info/maritial_status' and value_text= '%s' and xform_id = 483"%(d['member_info/maritial_status'])
            single = json.loads(row_query(single_query))
            if single is not '':
                d['member_info/maritial_status'] = str(single['Bangla'].encode('utf-8'))
        if 'member_info/relation' in d:
            relation_query = "select field_detail->>'label' from xform_extracted where field_name='member_info/relation' and value_text= '%s' and xform_id = 483"%(d['member_info/relation'])
            relation = json.loads(row_query(relation_query))
            if relation is not '':
                d['member_info/relation'] = str(relation['Bangla'].encode('utf-8'))
        
        if 'member_info/involement_iga' in d:
            #d['member_info/involement_iga']= ''
            occupation = d['member_info/involement_iga'].split(' ')
            org_string = ' , '.join("'"+str(x)+"'" for x in occupation)
            involement_iga_query = "select field_detail->>'label' from xform_extracted where field_name='member_info/involement_iga' and value_text in (%s) and xform_id = 483"%(org_string)
            print involement_iga_query
            #print type(row_query(involement_iga_query))
            involement_iga = __db_fetch_values(involement_iga_query)
            occupation = json.loads(involement_iga[0][0])['Bangla']
            d['member_info/involement_iga'] = str(occupation.encode('utf-8'))
        print "#######################3"
        print d
        renew_data.append(d)
    return renew_data
...
def save_user(request):
    ...
    if data['Role']:
        role = data['Role']
        Role = OrganizationRole.objects.filter(role =role).first()
    ...
    userRole_flag = 0
    user_pre = User.objects.filter(username=data['UserName']).first()
    if user_pre is not None:
        profile_pre = UserModuleProfile.objects.filter(user_id=user_pre.id).first()
        user_role_pre = UserRoleMap.objects.filter(user_id = profile_pre.id).first()
        user_form = UserForm(data=submitted_data,instance = user_pre)
        profile_form = UserProfileForm(data=submitted_data, admin_check=True, instance = profile_pre)
        userRole_flag = 1
    ...
    if user_form.is_valid() and profile_form.is_valid():
        ...
        organization = Organizations.objects.filter(organization = submitted_data['organization']).first()
        if userRole_flag == 1:
           pro = ProfileOrganization.objects.filter(profile_id=profile.id).first().delete()
           print "deleted"
	...
	userRole_form = UserRoleMapForm(data=submitted_data)
        if userRole_flag == 1:
      	    userRole_form = UserRoleMapForm(data=submitted_data, instance = user_role_pre)
            print userRole_form
	if userRole_form.is_valid():
	    userRole_form.save()
	else:
	    print userRole_form.errors
	    return HttpResponse('Role error', status=409)
...
def mobile_access(request, username):
    # username = request.username
    user = get_object_or_404(User, username=username)
    profile = get_object_or_404(UserModuleProfile, user=user)
    print profile.id
    roles = UserRoleMap.objects.filter(user_id=profile.id)

    # mobile_module = MobileModule.objects.filter(module_name=module_name)
    # print mobile_module
    access_json = []
    print "mobile access"
    for r in roles:
        print r.role
        try:
            module = MobileModuleAccess.objects.filter(role_id=r.role.id)
            for m in module:
                if m.edit == 1:
                    access_json.append('canEdit'+m.module.module_name)
                else:
                    if m.view == 1:
                        access_json.append('canView' + m.module.module_name)
            # return HttpResponse('success', status=200)
            return access_json
        except MobileModuleAccess.DoesNotExist:
            print ("User doesnot have access to this module")
            return access_json
...
def get_user_list(request, username):
    ...
    org_id_list = get_organization_by_user(user)
    profile_organization = ProfileOrganization.objects.filter(organization_id__in=org_id_list)
    profile_list = [pro.profile for pro in profile_organization]
    request_user_role = get_user_role(username)
    # 
    allowed_user = []
    if request_user_role=='RM':
        allowed_user=['BM', 'PO']
    if request_user_role=='BM':
        allowed_user.append('PO')
    for pro in profile_organization:
        p = pro.profile
        #print p.user.username
        user_role=get_user_role(p.user.username)
        if user_role in allowed_user:
            user_data = {}
            user_data['role'] = user_role
            user_data['user_name']=p.user.username
            user_data['first_name'] = p.user.first_name
            user_data['last_name'] = p.user.last_name
            user_data['email'] = p.user.email
            user_data['position'] = p.position
            user_data['employee_id'] = p.employee_id
            user_data['country'] = p.country
            user_data['is_admin'] = p.admin
            user_data['organization'] = pro.organization.organization
