...
def get_hh_list(request,username):
    ...
    sql = 'with t as(SELECT hh_status_id,  hh_serial, hh_id, village_id, spot_id, hh_name, nid, hh_husband_father_name,wealth_rank,mobile,gps,(SELECT status FROM public.approval_instanceapproval where instance_id=household.last_instance_id limit 1) status, hh_name FROM public.household where branch_id in(%s)) select t.*, dist_code, district, up_code, upazila , union_code, union_name, village_code, village,spot_code,spot from t,vwspot where t.spot_id=vwspot.spot_id'%(org_string)
    print sql
    household_list = __db_fetch_values(sql)
    json_data_response_all = []
    if len(household_list) > 0:
