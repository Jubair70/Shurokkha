...
def farmer_index(request):
    query = "select * from vwfarmer"
    paravet_list = []

    return render(request, 'shmodule/farmer_list.html', {'paravet_list':paravet_list})

def farmer_list(reguest):
    query = "select farmer_name, mobile, submitted_by, submission_time  from vwfarmer"
    df = pandas.read_sql(query , connection)
    # print(df.to_dict('record'))
    hh_list_data = map(list, df.values)
    col_names = list(df.columns.values)
    col_names.append("profile")

    return HttpResponse(json.dumps({'col_name': col_names, 'hh_list': hh_list_data}))
