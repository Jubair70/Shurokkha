...
from onadata.apps.usermodule.models import UserModuleProfile, UserPasswordHistory, UserFailedLogin,Organizations,OrganizationDataAccess, ProfileOrganization
...
def get_hh_list(request,username):
    ...
    sql = "select json->>'hh_id' item_id,json->>'_xform_id_string' form_id, json item_content,(SELECT status FROM public.approval_instanceapproval where instance_id=logger_instance.id limit 1) status from logger_instance where xform_id=459 and user_id=(select id from auth_user where username='" + username + "') and deleted_at is null order by id"
...
def get_pra_list(request,username):
    ...
    sql = "select json->>'pra_no' item_id,json->>'_xform_id_string' form_id,json item_content,(SELECT status FROM public.approval_instanceapproval where instance_id=logger_instance.id limit 1) status from logger_instance where xform_id=455 and user_id=(select id from auth_user where username='" + username + "') and deleted_at is null order by id"
...
def get_hh_profile(request,username,hhid):
    ...
    sql = "select id,json item_content from logger_instance where xform_id=457 and json->>'info/hh_id'='"+ hhid +"' order by id desc limit 1"
    ...
    if tmp_db_value is not None:
       ...
       for every in tmp_db_value: 
           print every[0]
           instance_data_json = {}
           instance_data_json['item_content'] = str(json.dumps(every[1]).encode('utf-8'))
           print instance_data_json
