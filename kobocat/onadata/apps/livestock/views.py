...
def get_total_ai(request,role_id,from_date="7/1/2019",to_date="7/1/2019"):
    ...
    total_ai = __db_fetch_single_value_excption("with t1 as (select distinct system_id cattle_id,_submitted_by submitted_by from vwreproduction where ai_or_pregnancy_or_delivery='1' and user_type = 'AI Technicians')select count(distinct cattle_id) from t1 " + sub_query)
    return total_ai
...
def get_total_pregnant_cattle(request,role_id,from_date="7/1/2019",to_date="7/1/2019"):
    ...
    total_pregnant_cattle = __db_fetch_single_value_excption("with t1 as (select distinct system_id cattle_id,ai_or_pregnancy_or_delivery ai_status,_submitted_by submitted_by from vwreproduction where ai_or_pregnancy_or_delivery = '2' and is_pregnant = '1' and user_type = 'AI Technicians')select count(distinct cattle_id) from t1"+ sub_query)
...
def ai_dashboard_content(request):
    ...
    print "total pregnant: " + dt.now().strftime("%Y-%m-%d %H:%M:%S")
    ...
    print "total ai done: " + dt.now().strftime("%Y-%m-%d %H:%M:%S")
