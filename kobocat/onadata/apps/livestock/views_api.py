...
def get_prescription_details(request,prescription_id):
    ...
    prescription_html = '<html><head> <title></title><link rel="stylesheet" type="text/css" href="https://cdnjs.cloudflare.com/ajax/libs/twitter-bootstrap/3.3.7/css/bootstrap.min.css"></head><body style="margin-left:15px;"> <div class="col-12"> <h3>Prescription:</h3> <p>খামারী : '+str(farmer_name.encode('utf-8'))+', তারিখ: '+str(created_date.encode('utf-8'))+'</p><p>গরু: '+str(cattle_type.encode('utf-8'))+', '+str(age_year.encode('utf-8'))+' বছর '+str(age_month.encode('utf-8'))+' মাস '+str(age_day.encode('utf-8'))+' দিন</p><h3>Rx:</h3> '+str(medicine_str.encode('utf-8'))+' </div><div class="col-12"> <h3>Advice:</h3> <p>'+str(advice.encode('utf-8'))+'</p><h4>'+str(next_appointment_after)+' দিন পর পুনরায় তথ্য পাঠিয়ে ডাক্তারের সাথে যোগাযোগ করুন</h4> </div><div class="col-md-2 col-sm-12" style=" text-align: center; padding-bottom: 100px; float: right"> <img src="'+str(signature_img)+'" width="200" height="74"> <br>'+str(prescribey_by.encode('utf-8'))+' </div><div class="col-md-12 col-sm-12" style=" text-align: center; padding: 50px;"> জরুরী প্রয়জনে ০১২৪৫৮৭৯৬৫২ নাম্বারে যোগাযোগ করুন </div></body></html>'
