...
def get_hh_list(request,username):
    ...
    if spot_string == '':
	spot_string = "'%'"
    sql = 'SELECT hh_status_id,  hh_serial, hh_id, village_id, spot_id, beneficiary_name, nid, hh_husband_father_name,wealth_rank,mobile,gps,(SELECT status FROM public.approval_instanceapproval where instance_id=household.last_instance_id limit 1) status, hh_name FROM public.household where village_id in (%s)'%(spot_string)
    print sql
    ...
    if len(household_list) > 0:
        for household in household_list:
            ...
            json_data_response['hh_head'] = household[12]
            ...
            json_data_response['status'] = household[11]
	    data = __db_fetch_values("select dist_code, district, up_code, upazila , union_code, union_name, village_code, village from vwvillage where village_id = '%s'" % (household[3]))
            ...
            spots = __db_fetch_values("select spot_code,spot from vwspot where spot_id = '%s'" % (household[4]))
            if len(spots)>0:
                spot = village+spots[0][0]
...
def household_data(hhid, json_data_response):
    ...
    ben_query = "select json->>'husband_father_name' , json->>'beneficiary_name', json->>'member_info', json->>'mobile', json->>'group_selected_assessment', json->>'picture', (select username from auth_user where id = user_id), json->>'gps', json->>'type_guardian' from vwlogger_instance where xform_id = 457 and json->>'info/hh_id' = '%s'" % (
        hhid)
    print ben_query
    enterprise_query = "select json->>'enterprise_name',json->>'enterprise_option',json->>'enterprise_type' from vwlogger_instance where xform_id = 464 and json->>'info/hh_id' = '%s'" % (
        hhid)
