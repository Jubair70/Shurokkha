...
function filterFarmerList() {
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            for (var i = 0; i < result.hh_list.length; i++) {
                row = result.hh_list[i];
                // generateHHListTable(result.hh_list);
                for (j=0 ;j< row.length; j++){
                    row[j] = checkNullVal(row[j]);
                }
                //console.log(row)
                // row.push('<a href="#" class="btn red" onclick="getProfile(\''+row[0]+'\')" role="button">Profile</a>');
                rowData.push(row)
                console.log("filtered");
            ...
            initDataTable("farmer_table", rowData, result.col_name);
    ...
    if ($.fn.dataTable.isDataTable('#' + tableID)) {
        ...
        $('#' + tableID).DataTable({
            ...
            "columnDefs": [{
                ...
                {   console.log("Test");
                    return '<a class="btn red" onclick="getProfile(\'' + row[0] + '\')" role="button">Profile</a>';
                }
            }],
...
//             "columnDefs":[{"targets":6, "data":"", "render": function(data,type,full,meta)
