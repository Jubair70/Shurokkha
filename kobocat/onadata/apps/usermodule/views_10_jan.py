...
def get_hh_list(request, username):
    ...
    if len(household_list) > 0:
	    ...
	    json_data_response['wealth_rank'] = row_query(wealth_rank_query)
            ...
            json_data_response['form_id'] = 'hh_registration'
            '''last visit date'''
            visit_plan_query = "select json->>'date' from vwlogger_instance where xform_id = 484 and json->>'hh_id'='%s' order by json->>'date' desc" % (household[2])
            print visit_plan_query
            visit_plan = __db_fetch_values(visit_plan_query)
            print visit_plan
            json_data_response['visit_plan_date'] = ''
...
def get_asset_list(request,username):
    ...
    sql = "select json->>'info/hh_id', json->>'_xform_id_string', json->>'asset_type', json->>'value_tk', json->>'date_asset_distribution', json->>'asset_name', json->>'asset_quantity', (SELECT status FROM public.approval_instanceapproval where instance_id=logger_instance.id limit 1) status from logger_instance where xform_id=471 and user_id=(select id from auth_user where username='" + username + "') and deleted_at is null order by id"
...
def household_data(hhid, json_data_response):
    ...
    if len(enterprise) > 0:
        ...
        if enterprise[0][2] is not None:
            ...
            json_data_response['enterprise_type_name'] = enterprise_type
        if enterprise[0][1] is not None:
            enterprise_name_query = "select name from  enterprise where code = %s" % (int(enterprise[0][0]))
            enterprise_name = row_query(enterprise_name_query)
            json_data_response['enterprise_name_id'] = int(enterprise[0][0])
            json_data_response['enterprise_name_name'] = enterprise_name
        if enterprise[0][0] is not None:
            enterprise_option_query = "select option_name from  enterprise_option where code = %s" % (int(enterprise[0][1]))
            enterprise_option = row_query(enterprise_option_query)
            json_data_response['enterprise_option_id'] = int(enterprise[0][1])
            json_data_response['enterprise_option_name'] = enterprise_option
