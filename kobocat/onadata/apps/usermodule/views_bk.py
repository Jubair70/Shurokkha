...
def get_hh_datalist(request,username,hhid):
    ...
    sql = "select xml xml_content,json->>'_xform_id_string' form_id,to_char(date_created,'DD-Mon-YYYY HH:mm') created_date,(select title from public.logger_xform where id = logger_instance.xform_id limit 1) form_name from logger_instance where (json->>'hh_id'='" + hhid + "' or json->>'info/hh_id'='" + hhid + "') and deleted_at is null order by id"
    ...
    if tmp_db_value is not None:
       ...
       for every in tmp_db_value: 
           ...
           instance_data_json['created_date'] = str(every[2])
