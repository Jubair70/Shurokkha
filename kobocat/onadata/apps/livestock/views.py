...
def advisory_list1(request):
    return render(request, 'livestock/advisory_list1.html')
...
def advisory_list(request):
    ...
    query = "with t as( select id,(select created_date from prescription where appointment_id = appointment.id) prescription_date,cattle_system_id,(select (json->>'cattle_type') cattle_type from logger_instance where id = healthrecord_sickness_system_id),(select (json->>'mobile') mobile from logger_instance where id = healthrecord_sickness_system_id),(select (json->>'_submitted_by') submitted_by from logger_instance where id = healthrecord_sickness_system_id),status from appointment where appointment_type =any('{1,3}')) select id,cattle_system_id,(select first_name || ' ' || last_name from auth_user where username = t.mobile) farmer_name,mobile,(select label from vwcattle_type where value = t.cattle_type) cattle_type,(select case when cattle_birth_date is null then cattle_age else AGE(current_date ,date(cattle_birth_date))::text end cattle_age from cattle where cattle_system_id = t.cattle_system_id),coalesce(substring(prescription_date::text from 0 for 20),'') prescription_date,(select first_name || ' ' || last_name from auth_user where username = t.mobile)ai_paravet_name,submitted_by,status from t where status  = any('{0,2}')"
...
def getAdvisoryData(request):
    ...
    query = "with t as( select id,created_date appointment_date,(select created_date from prescription where appointment_id = appointment.id) prescription_date,cattle_system_id,(select (json->>'cattle_type') cattle_type from logger_instance where id = healthrecord_sickness_system_id),(select (json->>'mobile') mobile from logger_instance where id = healthrecord_sickness_system_id),(select (json->>'_submitted_by') submitted_by from logger_instance where id = healthrecord_sickness_system_id),status from appointment where appointment_type =any('{1,3}')) select id,cattle_system_id,appointment_date::date,(select first_name || ' ' || last_name from auth_user where username = t.mobile) farmer_name,mobile,(select label from vwcattle_type where value = t.cattle_type) cattle_type,(select case when cattle_birth_date is null then cattle_age else AGE(current_date ,date(cattle_birth_date))::text end cattle_age from cattle where cattle_system_id = t.cattle_system_id),coalesce(substring(prescription_date::text from 0 for 20),'') prescription_date,(select first_name || ' ' || last_name from auth_user where username = t.mobile)ai_paravet_name,submitted_by,status from t where status = any('{0,2}') "+str(filter_query)
...
def sickness_list(request):
    ...
    ai_paravet_name = df.ai_paravet_name.tolist()
    ai_paravets = zip(ai_paravet_id,ai_paravet_name)
    query = "with t as( select id,(select created_date from prescription where appointment_id = appointment.id) prescription_date,cattle_system_id,(select (json->>'cattle_type') cattle_type from logger_instance where id = healthrecord_sickness_system_id),(select (json->>'mobile') mobile from logger_instance where id = healthrecord_sickness_system_id),(select (json->>'_submitted_by') submitted_by from logger_instance where id = healthrecord_sickness_system_id),status from appointment where appointment_type =2) select id,cattle_system_id,(select first_name || ' ' || last_name from auth_user where username = t.mobile) farmer_name,mobile,(select label from vwcattle_type where value = t.cattle_type) cattle_type,(select case when cattle_birth_date is null then cattle_age else AGE(current_date ,date(cattle_birth_date))::text end cattle_age from cattle where cattle_system_id = t.cattle_system_id),coalesce(substring(prescription_date::text from 0 for 20),'') prescription_date,(select first_name || ' ' || last_name from auth_user where username = t.mobile)ai_paravet_name,submitted_by,status from t where status  = any('{0,2}')"
...
def getSicknessData(request):
    ...
    if ai_paravet != "":
        filter_query += "and submitted_by = '" + str(ai_paravet) + "' "
    query = "with t as( select id,created_date appointment_date,(select created_date from prescription where appointment_id = appointment.id) prescription_date,cattle_system_id,(select (json->>'cattle_type') cattle_type from logger_instance where id = healthrecord_sickness_system_id),(select (json->>'mobile') mobile from logger_instance where id = healthrecord_sickness_system_id),(select (json->>'_submitted_by') submitted_by from logger_instance where id = healthrecord_sickness_system_id),status from appointment where appointment_type = 2) select id,cattle_system_id,appointment_date::date,(select first_name || ' ' || last_name from auth_user where username = t.mobile) farmer_name,mobile,(select label from vwcattle_type where value = t.cattle_type) cattle_type,(select case when cattle_birth_date is null then cattle_age else AGE(current_date ,date(cattle_birth_date))::text end cattle_age from cattle where cattle_system_id = t.cattle_system_id),coalesce(substring(prescription_date::text from 0 for 20),'') prescription_date,(select first_name || ' ' || last_name from auth_user where username = t.mobile)ai_paravet_name,submitted_by,status from t where status = any('{0,2}') "+str(filter_query)
    data = json.dumps(__db_fetch_values_dict(query), default=decimal_date_default)
    return HttpResponse(data)