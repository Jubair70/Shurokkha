...
def get_hh_list(request, username):
    ...
    #     spot_string = "'%'"
    # sql = 'SELECT hh_status_id,  hh_serial, hh_id, village_id, spot_id, hh_name, nid, hh_husband_father_name,wealth_rank,mobile,gps FROM public.household where village_id in (%s)'%(spot_string)
    sql = 'with t as(SELECT hh_status_id,  hh_serial, hh_id, village_id, spot_id, hh_name, vo_id, hh_husband_father_name,wealth_rank,(select cohort_name from cohort where id =household.cohort_id limit 1 ) as cohort ,gps,(SELECT status FROM public.approval_instanceapproval where instance_id=household.last_instance_id limit 1) status, household.last_instance_id FROM public.household where branch_id in(%s)) select t.*, dist_code, district, up_code, upazila , union_code, union_name, village_code, village,spot_code,spot from t,vwspot where t.spot_id=vwspot.spot_id' % (
    org_string)
    print sql
    household_list = __db_fetch_values(sql)
    json_data_response_all = []
...
def get_training_list(request,username):
    ...
    query_po = "select first_name, last_name from auth_user where username='"+username+"'"
    ...
    if len(training_list) > 0:
        for training in training_list:
            ...
            enterprise_name_query = "select name from  enterprise where id = %s" % (int(training[2]))
	    ...
	    if training[3] is not None:
                enterprise_option_query = "select option_name from  enterprise_option where id = %s" % (int(training[3]))
            ...
            group_value_query = "select value_label from xform_extracted where xform_id = 458 and field_name = 'group' and value_text = '%s'" % (training[7])
...
def save_user(request):
    ...
    if 'EmployeedId' in data:
	submitted_data['employee_id'] = data['EmployeedId']
    if 'Position' in data:
