...
class UserForm(forms.ModelForm):
    password = forms.CharField(required=True,label='Create a password',widget=forms.PasswordInput(),min_length=4)
    password_repeat = forms.CharField(required=True,label='Confirm your password',widget=forms.PasswordInput())
...
class UserProfileForm(forms.ModelForm):
    ...
    #country = forms.ChoiceField(choices=COUNTRIES, required=False, label='Country')
    ...
    class Meta:
        ...
        fields = ('admin','organisation_name','contact_number')
...
class OrganizationForm(forms.ModelForm):
    organization = forms.CharField(label='Farm Name',required=True,validators=[RegexValidator('^[a-zA-Z0-9 ]*$', message="Farm name can only be Alphabets and Numbers")])
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['organization'].error_messages = {'max_length': 'Please provide a value of no more  than 40 characters.','required':'The field is required'}
